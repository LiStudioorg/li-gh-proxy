...
description: 在 K3s、RKE2 及原生 containerd 中配置 li-gh-proxy 镜像加速。
---

li-gh-proxy 兼容 OCI/Distribution Registry API v2。Kubernetes 节点通常由 **containerd** 拉取镜像，配置方式与 Docker `daemon.json` 不同。
...
- 每节点独立配置；生产环境 li-gh-proxy 建议 HTTPS 反代
- li-gh-proxy 对上游匿名拉取，**无法**代理需要认证的私有镜像
