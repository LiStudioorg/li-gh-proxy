...
description: li-gh-proxy security model, risks, and best practices.
---

li-gh-proxy is an open proxy service — improper deployment creates abuse risk. Follow these practices to reduce exposure.
...
li-gh-proxy has **no** admin login, Basic Auth, API keys, or Prometheus `/metrics`. The web UI is a public SPA — security relies on network placement and configuration.
...
- Use [Recommended Architecture](/en/deployment/architecture/): CDN (optional) → reverse proxy → li-gh-proxy
