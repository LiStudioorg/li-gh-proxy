---
title: IP Trust
description: How li-gh-proxy identifies real client IPs and prevents spoofing.
---

li-gh-proxy uses Gin's `ClientIP()` with trusted proxy CIDRs to decide when to trust forwarding headers.
...
li-gh-proxy does **not** ship a Cloudflare IP trust list — it relies on the private-side proxy writing the correct IP.
