...
description: Complete reference for the li-gh-proxy configuration file.
---
...
| `fileSize` | int / string | `2147483648` | Max single-file size, GitHub / Hugging Face URL proxy only; plain byte count or unit strings like `"2GB"`, `"1.5GiB"` |
...
:::note
All registries currently use anonymous pulls (`authn.Anonymous`). `authType` labels the auth endpoint only — **GitHub tokens and Google service accounts are not yet implemented**, and client `Authorization` headers are not forwarded. **Private authenticated images cannot be pulled through li-gh-proxy.**
:::
...
| `defaultTTL` | string (duration) | `"20m"` | Default manifest cache TTL for ordinary tags, e.g. `"20m"`, `"1h30m"`; validated at startup |
...
List of acceleration nodes (alternative deployment addresses of this service). Once configured, they are shown on the frontend home page for users to switch between; generated accelerated links and `docker pull` examples will use the selected node's domain. The user's choice is stored in browser `localStorage` (key: `node`). Changes take effect after restart.

Each node entry:

| Key | Type | Description |
|-----|------|-------------|
| `name` | string | Display name; if empty, the `url` hostname is used automatically |
| `url` | string | Full node address, must start with `http://` or `https://`; invalid values cause startup failure |
...
:::note
When the list is empty (the default), the node switcher is not rendered and all links use the current site's domain. The node list is served via `GET /api/nodes`.
:::
...
| `GET /api/nodes` | Acceleration node list (`current` is the current request Host) |
