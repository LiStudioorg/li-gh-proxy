...
The reverse proxy must **overwrite** the real client IP so li-gh-proxy can rate-limit per user and resist IP spoofing.
...
li-gh-proxy only trusts forwarding headers when the TCP connection comes from trusted private/local ranges:
