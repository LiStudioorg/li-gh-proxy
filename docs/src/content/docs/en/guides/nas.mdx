...
description: Configure li-gh-proxy on Synology DSM and Feiniu fnOS.
---

Do **not** overwrite entire `daemon.json` with scripts — you may lose `data-root` and break existing containers.

Assumes li-gh-proxy is exposed at `https://example.com` via reverse proxy.
...
- App Center apps may use fixed registries (e.g. Aliyun) unrelated to li-gh-proxy
