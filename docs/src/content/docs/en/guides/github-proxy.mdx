...
description: Accelerate GitHub Release, Raw, Clone, API, and Hugging Face downloads with li-gh-proxy.
---

Requests that do not match `/v2`, `/token`, `/api`, or other registered routes are handled by the GitHub proxy. Prefix the **full original URL** with your li-gh-proxy host; if the URL matches a supported pattern, the handler reconstructs it and proxies to GitHub, Hugging Face, or other upstreams — otherwise it returns "invalid input".

## How to Accelerate

Prefix your original URL with the li-gh-proxy host. Two formats are supported.
...
Use `url.<base>.insteadOf` so every `https://github.com/` clone/fetch goes through li-gh-proxy automatically:
...
li-gh-proxy auto-prepends the scheme:
...
li-gh-proxy can rewrite GitHub URLs inside `.sh` / `.ps1` install scripts (max 10MB). Behind a reverse proxy, set `X-Forwarded-Host` correctly — see [Reverse Proxy](/en/deployment/reverse-proxy/).
