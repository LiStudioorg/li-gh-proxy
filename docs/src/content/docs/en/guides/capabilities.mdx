...
description: Transfer behavior for Docker pulls and GitHub downloads in li-gh-proxy.
---
...
li-gh-proxy fetches manifests and blobs via go-containerregistry, then returns them over HTTP.
...
| Auth realm rewrite | ✅ | Upstream token → li-gh-proxy `/token` |
