---
title: FAQ
description: Frequently asked questions about deploying and using li-gh-proxy.
---

<details>
<summary>How is li-gh-proxy different from free CDN proxies?</summary>

li-gh-proxy is fully self-hosted — you control data and bandwidth without relying on unstable third-party CDNs. Customize rate limits, registry allowlists, and upstream SOCKS5 proxies.
...
Prefix the **full original URL** with your li-gh-proxy host, e.g. `https://example.com/https://github.com/owner/repo/releases/download/...`. Path-prefix rewrites like `/github/owner/...` are not supported. See [GitHub Acceleration](/en/guides/github-proxy/).
...
If li-gh-proxy connects directly to Cloudflare edges, rate limits apply per Cloudflare IP, not per user. Add a private reverse proxy that writes `CF-Connecting-IP` to forwarding headers.
...
<details>
<summary>Does li-gh-proxy support resume or multi-part downloads?</summary>
...
`authType` labels auth endpoints only — credential injection is not yet implemented. All registry pulls use anonymous auth and client `Authorization` headers are not forwarded. **Private authenticated images cannot be pulled through li-gh-proxy.**
