---
title: 安全概述
description: li-gh-proxy 安全模型、风险点与最佳实践。
---

li-gh-proxy 作为开放代理服务，部署不当可能带来滥用风险。遵循以下实践可显著降低暴露面。
...
li-gh-proxy **没有**管理后台登录、Basic Auth、API Key 或 Prometheus 指标端点。Web 界面为公开 SPA，安全依赖网络层与配置策略。
...
- 使用 [推荐部署架构](/deployment/architecture/)：CDN（可选）→ 反代 → li-gh-proxy
